...
public class UserRepository : IUserRepository
{
    ...
    public async Task<Result<Tokens>> DeleteRefreshTokenAsync(
        string refreshToken,
        int userId,
    ...
    )
    {
        ...
        try
        {
            var item = await _context.Tbl_Login.FirstOrDefaultAsync(
                x => x.RefreshToken == refreshToken && x.UserId == userId,
    ...
    )
    {
        ...
        try
        {
            var user = await _context.Tbl_User.FirstOrDefaultAsync(
    ...
    public async Task SaveRefreshTokenAsync(int userId, string refreshToken, CancellationToken cs)
    {
        try
        {
            ...
            var login = await _context.Tbl_Login.FirstOrDefaultAsync(
                x => x.UserId == userId,
                cancellationToken: cs
            );
            ...
            if (login is null)
            {
                var model = new Tbl_Login { UserId = userId, RefreshToken = refreshToken };
    ...
    private JwtResponseModel GetJwtResponseModel(Tbl_User tbl_User) =>
        new(
